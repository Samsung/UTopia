//===- CustomCVP.cpp - Propagate called values -----*- C++ -*-===//
//
...
/// possible targets, anyway. CustomCVP changed this value.
/// It's used only as an analysis result (target candidate list),
/// so there are no side effects.
static cl::opt<unsigned> MaxFunctionsPerValue(
    "ccvp-max-functions-per-value", cl::Hidden, cl::init(20),
...
public:
  ...
  /// LLVM CVP track local Global Variables,
  /// variables used as arguments of local function.
  /// Custom CVP removes the condition related to local attribute
  /// to track more variable.
  CVPLatticeVal ComputeLatticeVal(CVPLatticeKey Key) override {
    ...
    case IPOGrouping::Return:
      ...
      } else if (auto *F = dyn_cast<Function>(Key.getPointer()))
  ...
  /// overdefined.
  /// LLVM CVP has set overdefined if X or Y are overdefined when merging,
  /// but Custom CVP has set overdefined when X and Y are overdefined.
  /// Changed condition helps to keep track of more variables.
  ...
  void ComputeInstructionState(
    ...
    case Instruction::Invoke: {
      ...
      return visitCallBase(*CB, ChangedValues, SS);
    }
...
private:
  ...
  void visitCallBase(CallBase &CB,
                     DenseMap<CVPLatticeKey, CVPLatticeVal> &ChangedValues,
                     SparseSolver<CVPLatticeKey, CVPLatticeVal> &SS) {
    Function *F = util::getCalledFunction(CB);
  ...
  /// CustomCVP take the return value of stripPointerCasts()
  /// instead of pure operand value.
  /// stripPointerCasts returns the original uncasted value.
  /// According to the evaluation result of CustomCVP,
  /// this value is more useful.
  void visitLoad(LoadInst &I,
                 DenseMap<CVPLatticeKey, CVPLatticeVal> &ChangedValues,
                 SparseSolver<CVPLatticeKey, CVPLatticeVal> &SS) {
    Value *SOP = I.getPointerOperand()->stripPointerCasts();
    auto RegI = CVPLatticeKey(&I, IPOGrouping::Register);
    if (auto *GV = dyn_cast<GlobalVariable>(SOP)) {
  ...
  void visitStore(StoreInst &I,
                  ...
                  SparseSolver<CVPLatticeKey, CVPLatticeVal> &SS) {
    Value *SOP = I.getPointerOperand()->stripPointerCasts();
    auto *GV = dyn_cast<GlobalVariable>(SOP);
    if (!GV)
      return;
...
template <> struct LatticeKeyInfo<ftg::CVPLatticeKey> {
  static inline Value *getValueFromLatticeKey(ftg::CVPLatticeKey Key) {
    return Key.getPointer();
  }
  static inline ftg::CVPLatticeKey getLatticeKeyFromValue(Value *V) {
    return ftg::CVPLatticeKey(V, ftg::IPOGrouping::Register);
  }
};
} // namespace llvm
...
static bool runCVP(Module &M) {
  ...
  // CustomCVP take the value of stripPointerCasts instead of pure CalledValue.
  // According to evaluation result of CustomCVP, this value is more useful.
  bool Changed = false;
  MDBuilder MDB(M.getContext());
  for (Instruction *C : Lattice.getIndirectCalls()) {
    auto *CB = dyn_cast_or_null<CallBase>(C);
    if (!CB)
      continue;
    ...
    CO = CO->stripPointerCasts();
    if (!CO)
      continue;

    auto RegI = CVPLatticeKey(CO, IPOGrouping::Register);
